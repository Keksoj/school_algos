...
def _onConnect(client, agent, flags, rc):
    ...
    if (rc == 0):
        ...
        if (agent.verbose):
            print("Abonnement de", agent.nom, "au topic",
                  agent._topicAgentsRead, "en cours ...")
        agent._mqtt.subscribe(agent._topicAgentsRead)
        if (agent.verbose):
            print("Abonnement de", agent.nom, "au topic",
                  agent._topicGameRead, "en cours ...")
        agent._mqtt.subscribe(agent._topicGameRead)
    else:
        print("Connexion de " + agent.nom +
              " au serveur retourne code", str(rc))
...
def _onMessage(client, agent, msg):
    ...
    # Reception de nouvel etat du jeu
    if (msg.topic == agent._topicGameRead):
        ...
        if (jeuAChange):
            voisinsASupprimer = []
            for voisin in agent.voisins:
                agentActuelVoisin = False
                for agentActuel in state["agents"]:
                    if (agentActuel == voisin):
                        agentActuelVoisin = True
                        break
                if (agentActuelVoisin == False):
                    voisinsASupprimer.append(voisin)
            for voisinASupprimer in voisinsASupprimer:
                del agent.voisins[voisinASupprimer]
            agent.quandJeuChange(agent.jeu, state)
            if (agent._quandJeuChange != None):
                agent._quandJeuChange(agent, agent.jeu, state)
            agent.jeu = state

    # Reception de nouvel etat de l'agent
        ...
        if ('orientation' in state):
            nouvelleOrientation = int(state['orientation'])
            if (nouvelleOrientation != agent.orientation):
                agent.quandOrientationChange(
                    agent.orientation, nouvelleOrientation)
                if (agent._quandOrientationChange != None):
                    agent._quandOrientationChange(
                        agent, agent.orientation, nouvelleOrientation)
            agent.orientation = nouvelleOrientation
            agent.pose = (agent.x, agent.y, agent.orientation)
        if ('vie' in state):
            nouvelleVie = int(state['vie'])
        ...
        if ('munitions' in state):
            nouvellesMunitions = int(state['munitions'])
            if (nouvellesMunitions != agent.munitions):
                ...
                if (agent._quandMunitionsChange != None):
                    agent._quandMunitionsChange(
                        agent, agent.munitions, nouvellesMunitions)
            agent.munitions = nouvellesMunitions
        if ('distance' in state):
            nouvelleDistance = int(state['distance'])
            if (nouvelleDistance != agent.distance):
                ...
                if (agent._quandDistanceChange != None):
                    agent._quandDistanceChange(
                        agent, agent.distance, nouvelleDistance)
            agent.distance = nouvelleDistance
        if ('derniereDestinationAtteinte' in state):
            nouvelleDestinationAtteinte = str(
                state['derniereDestinationAtteinte'])
            if (nouvelleDestinationAtteinte != agent.derniereDestinationAtteinte):
                agent.quandDestinationAtteinte(
                    agent.derniereDestinationAtteinte, nouvelleDestinationAtteinte)
                if (agent._quandDestinationAtteinte != None):
                    agent._quandDestinationAtteinte(
                        agent, agent.derniereDestinationAtteinte, nouvelleDestinationAtteinte)
            agent.derniereDestinationAtteinte = nouvelleDestinationAtteinte
        if ('tir' in state):
            if (isinstance(state['tir'], bool)):
                agent.tir = state['tir']
            else:
                agent.tir = True
        if ('couleur' in state):
            agent.couleur = state['couleur']
        if (agent._premiereReception):
            agent._premiereReception = False
    # Reception des agents voisins
    else:
        topicsplit = msg.topic.split('/')
        topicAgentReadsplit = Agent.topicAgentRead.split('/')
        lensplit = len(topicAgentReadsplit)+2
        if (len(topicsplit) != lensplit):
            if (agent.verbose):
                print("ERREUR: Reception topic", msg.topic, "invalide")
            return
        nom = topicsplit[lensplit-1]
        agent.voisins[nom] = state
...
def _toRequest(agent):
    ...
    for cle, valeur in agent._ordre.items():
        request[cle] = valeur
    if (agent._premiereReception):
        if ('orientation' not in request):
            request['orientation'] = 0
    else:
        tir = agent.tir
        if ('tir' in agent._ordre):
            tir = agent._ordre['tir']
...
class Agent:
    ...
    def __init__(self, nom="", username="", password="", arene="", server="", prompt=True, verbose=False):
        """Valeurs possibles pour username, password, arene et server : chaines de caractères communiquées par l'administrateur jusdeliens
        Choix du nom à votre guise"""
        print("Bonjour à tous les jusdeliens ! Pour participer aux prochains affrontements, rendez-vous sur https://jusdeliens.com\nPour visualiser l'arène de jeu, allez sur https://jusdeliens.com/play/pytactx")
        if (type(nom) is not str or type(username) is not str or type(password) is not str or type(arene) is not str or type(server) is not str):
            print("ATTENTION: utiliser des texte entre guillement (str) pour les parametres nom,username,password,server pour construire un objet Agent")
            nom = str(nom)
        ...
        if (type(verbose) is not bool or type(prompt) is not bool):
            print("ATTENTION: utiliser un booleen (bool) pour les parametres verbose et prompt pour construire un objet Agent")
            verbose = False

        # Prompt console si constructeur sans parametres
        if (nom == "PrenomNOM"):
            nom = input("Entrer le nom de votre agent: ")
        if (nom == "" and prompt == True):
            nom = input("Entrer le nom de votre agent: ")
        if (nom == ""):
            nom = "Agent" + str(random.randint(0, 9999))
        ...
        if (username == ""):
            username = "demo"
            password = "demo"
        if (arene == "" and prompt == True):
            arene = input("Entrer le nom de l'arene: ")
        if (arene == ""):
            arene = "demo"
        ...
        self.jeu = {"agents": [], "modeMove": "free", "destinations": {}, "dtOrientation": 1000, "dtMove": 1000, "dtPop": 15000, "dtFire": 1000, "damageFire": 10, "damageCollision": 10,
                    "dxMax": 1, "dyMax": 1, "gridColumns": 10, "gridRows": 10, "maxAgents": 8, "lifeIni": 100, "ammoIni": 10, "dtPing": 10000, "onlyAuthorised": False, "pause": False, "connected": False}
        self.voisins = {}
        self._ordre = {}
        ...
        self._topicGameRead = Agent.topicGameRead + "/" + arene
        self._topicAgentsRead = Agent.topicAgentRead + "/" + arene + "/+"
        self._topicAgentRead = Agent.topicAgentRead + "/" + arene + "/" + self.nom
        self._topicAgentWrite = Agent.topicAgentWrite + "/" + arene + "/" + self.nom
        self._qos = 0
    ...
    def actualiser(self):
        ...
        if ((self._premiereReception or len(self._ordre) > 0) and (dtEnvoi > Agent.dtEnvoiRequete or Agent.actualiserAvecSleep)):
    ...
    def tirer(self, activer=True, fonction=None):
        ...
        if (type(activer) is not bool):
            print(
                "ATTENTION: utiliser un paramètre booleen (bool) pour appeler la fonction tirer !!")
            activer = False
        if (activer == self.tir and fonction == self._fonctionTir):
            return False
        self._fonctionTir = fonction
        self._ordre['tir'] = activer
        return True

    def deplacerVers(self, destination):
        """Se déplacer vers un lieu au format str parmi jeu["destinations"] en mode jeu["modeMove"] destination"""
        ...
        if (type(destination) is not str):
            ...
            return False
        self._ordre['destination'] = destination
        if (destination not in self.jeu["destinations"]):
            return
        destinationX = self.jeu["destinations"][destination]['x']
        destinationY = self.jeu["destinations"][destination]['y']
    ...
    def deplacer(self, x, y):
        ...
        if (type(x) is not int or type(y) is not int):
            print(
                "ATTENTION: utiliser des x,y entiers (int) pour appeler la fonction deplacer !!")
            x = int(x)
            y = int(y)
        if (x < 0 or y < 0):
            return False
    ...
    def orienter(self, orientation):
        ...
        if (type(orientation) is not int):
            print(
                "ATTENTION: utiliser une orientationt entière (int) pour appeler la fonction orienter !!")
            orientation = int(orientation)
        if (orientation < 0 or orientation > 3):
            print(
                "ATTENTION: orientation doit être compris entre 0 et 3 pour appeler la fonction orienter !!")
            return False
        if (self.orientation == orientation):
            return False
        self._ordre['orientation'] = orientation
        return True
    ...
    def changerCouleur(self, rouge, vert, bleu):
        ...
        if (self.couleur[0] == rouge and self.couleur[1] == vert and self.couleur[2] == bleu):
            return False
        self._ordre['couleur'] = (rouge, vert, bleu)
